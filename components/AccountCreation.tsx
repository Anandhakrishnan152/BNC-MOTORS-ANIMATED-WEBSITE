            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="s1" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 gap-8">
                    <div className="space-y-3">
                      <label className="text-[9px] font-heading tracking-widest text-gray-400 uppercase font-black">IDENTIFIER_NAME</label>
                      <input type="text" placeholder="ADITYA_VERMA" className="w-full bg-slate-50 border border-black/5 px-8 py-5 font-heading text-xs uppercase tracking-widest focus:border-emerald-500 outline-none transition-colors" />
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        <label className="text-[9px] font-heading tracking-widest text-gray-400 uppercase font-black">NEURAL_EMAIL</label>
                        <input type="email" placeholder="USER@BNC.IO" className="w-full bg-slate-50 border border-black/5 px-8 py-5 font-heading text-xs uppercase tracking-widest focus:border-emerald-500 outline-none transition-colors" />
                      </div>
                      <div className="space-y-3">
                        <label className="text-[9px] font-heading tracking-widest text-gray-400 uppercase font-black">MOBILE_UID</label>
                        <input type="tel" placeholder="+91-0000000000" className="w-full bg-slate-50 border border-black/5 px-8 py-5 font-heading text-xs uppercase tracking-widest focus:border-emerald-500 outline-none transition-colors" />
                      </div>
                    </div>
                  </div>
                  ...
                  <button 
                    onClick={handleNext} 
                    disabled={loading}
                    className="w-full bg-black text-white py-7 font-heading text-[11px] font-black tracking-[0.5em] hover:bg-emerald-600 transition-all uppercase flex items-center justify-center gap-4"
                  >
              ...
              {step === 2 && (
                <motion.div key="s2" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  ...
                  <div className="space-y-3">
                    <label className="text-[9px] font-heading tracking-widest text-gray-400 uppercase font-black">VEHICLE_IDENTIFIER_VIN</label>
                    <input type="text" placeholder="BNC-S110-XXXX-XXXX" className="w-full bg-slate-50 border border-black/5 px-8 py-5 font-heading text-xs uppercase tracking-widest focus:border-emerald-500 outline-none transition-colors" />
                  </div>

                  <div className="flex gap-4">
                     <button onClick={() => setStep(1)} className="flex-1 border border-black/10 py-7 font-heading text-[10px] font-black tracking-widest uppercase hover:bg-slate-50">BACK</button>
                     <button onClick={handleNext} disabled={loading} className="flex-[2] bg-black text-white py-7 font-heading text-[10px] font-black tracking-widest uppercase hover:bg-emerald-600">{loading ? 'SCANNING...' : 'VALIDATE_SYNC'}</button>
                  </div>
                </motion.div>
              )}
              ...
              {step === 3 && (
                <motion.div key="s3" variants={containerVariants} initial="hidden" animate="visible" exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  ...
                  <div className="space-y-8">
                     <div className="space-y-3">
                        <label className="text-[9px] font-heading tracking-widest text-gray-400 uppercase font-black">PRIVATE_KEY_PASSWORD</label>
                        <input type="password" placeholder="MIN_12_CHARACTERS" className="w-full bg-slate-50 border border-black/5 px-8 py-5 font-heading text-xs uppercase tracking-widest focus:border-emerald-500 outline-none transition-colors" />
                     </div>
                     <div className="space-y-3">
                        <label className="text-[9px] font-heading tracking-widest text-gray-400 uppercase font-black">CONFIRM_PRIVATE_KEY</label>
                        <input type="password" placeholder="RE-ENTER_PASSWORD" className="w-full bg-slate-50 border border-black/5 px-8 py-5 font-heading text-xs uppercase tracking-widest focus:border-emerald-500 outline-none transition-colors" />
                     </div>
                  </div>
                  ...
                  <button onClick={() => alert('OWNER_IDENTITY_ESTABLISHED')} className="w-full bg-emerald-600 text-white py-7 font-heading text-[11px] font-black tracking-[0.5em] hover:bg-black transition-all uppercase shadow-xl shadow-emerald-500/20">
                    ESTABLISH_IDENTITY
                  </button>
                </motion.div>
              )}
